/**
 * Generate Thai ESGX Fund List from SEC API
 *
 * This script fetches all Thai ESGX funds from the SEC API and generates:
 * - docs/thai-esgx-funds-api.csv
 * - docs/thai-esgx-funds-api.md
 *
 * 100% API-based - NO dependency on manual CSV files
 *
 * Thai ESGX funds are identified by checking if fund names/IDs contain:
 * - "ESGX" or "ESG X" or "THAIESGX"
 */
...
function generateCSV(funds: ESGXFund[]): string {
...
function generateMarkdown(funds: ESGXFund[]): string {
  const lines = [
    '# Thai ESGX Funds (API-Generated)',
    '',
    `**Total Funds:** ${funds.length}`,
    `**Generated:** ${new Date().toISOString()}`,
    `**Source:** SEC Fund Factsheet API`,
    '',
    '**About Thai ESGX Funds:**',
    '- Tax benefit: Up to 30% of assessable income (max THB 300,000)',
    '- Investment period: May-June 2025 (special campaign)',
    '- Minimum holding period: 5 years',
    '- Investment focus: ≥65% in Thai stocks (SET/MAI) with outstanding ESG performance',
    '',
...
async function generateESGXFundList() {
  log('╔════════════════════════════════════════════════════════════════════════════╗', 'yellow');
  log('║                Generate Thai ESGX Fund List from SEC API                  ║', 'yellow');
  log('╚════════════════════════════════════════════════════════════════════════════╝\n', 'yellow');

  // Clear cache for fresh fetch
  clearCache();

  const esgxFunds: ESGXFund[] = [];
  ...
  try {
    ...
    logSection('Step 2: Fetching Thai ESGX Funds from Each AMC');
    ...
    for (let i = 0; i < amcs.length; i++) {
      ...
      try {
        ...
        // Filter Thai ESGX funds
        const esgxFundsForAMC = funds.filter(fund => {
          const isESGX = fund.proj_id?.includes('ESGX') ||
                         fund.proj_id?.includes('ESG X') ||
                         fund.proj_id?.includes('THAIESGX') ||
                         ...
                         fund.proj_name_en?.toUpperCase().includes('THAI ESGX') ||
                         fund.proj_abbr_name?.includes('ESGX') ||
                         fund.proj_abbr_name?.includes('THAIESGX');
          return isESGX;
        });

        // Add to list
        for (const fund of esgxFundsForAMC) {
        ...
        if (esgxFundsForAMC.length > 0) {
          log(`  ✓ Found ${esgxFundsForAMC.length} Thai ESGX funds`, 'green');
        }
    ...
    esgxFunds.sort((a, b) => a.symbol.localeCompare(b.symbol));
    log(`✓ Sorted ${esgxFunds.length} funds by symbol`, 'green');

    // Step 4: Generate CSV
    logSection('Step 4: Generating CSV File');
    const csv = generateCSV(esgxFunds);
    const csvPath = join(process.cwd(), 'docs', 'thai-esgx-funds-api.csv');
    ...
    const markdown = generateMarkdown(esgxFunds);
    const mdPath = join(process.cwd(), 'docs', 'thai-esgx-funds-api.md');
    ...
    log(`Thai ESGX Funds: ${stats.esgx_funds}`, 'green');
    log(`  ├─ Active: ${stats.active_funds}`, 'green');
    log(`  └─ Cancelled: ${stats.cancelled_funds}`, 'yellow');

    logSection('Success!');
    log('Thai ESGX fund list generated from SEC API (100% API-based)', 'green');
